...
class ImageViewer:
    ...
    def handle_mouse_click(self, pos):
        ...
        if hasattr(self, "rows_input_rect") and self.rows_input_rect.collidepoint(pos):
            self.rows_input_focused = True
            self.cols_input_focused = False
        elif hasattr(self, "cols_input_rect") and self.cols_input_rect.collidepoint(
            pos
        ):
            self.cols_input_focused = True
            self.rows_input_focused = False
        elif hasattr(self, "apply_button_rect") and self.apply_button_rect.collidepoint(
            pos
        ):
            self.apply_grid()
        elif hasattr(
            self, "toggle_grid_button_rect"
        ) and self.toggle_grid_button_rect.collidepoint(pos):
            self.show_grid_overlay = not self.show_grid_overlay
        else:
            self.rows_input_focused = False
            self.cols_input_focused = False
    ...
    def draw_controls(self):
        ...
        pygame.draw.line(
            ...
            self.colors["border"],
            (0, panel_rect.top),
            (self.window_size[0], panel_rect.top),
            2,
        )
        ...
        border_color = (
            self.colors["accent"] if self.rows_input_focused else self.colors["border"]
        )
        pygame.draw.rect(
            self.screen, border_color, self.rows_input_rect, border_radius=4
        )
        pygame.draw.rect(
            self.screen,
            self.colors["input_bg"],
            self.rows_input_rect.inflate(-2, -2),
            border_radius=4,
        )
        ...
        self.screen.blit(
            rows_surf, (self.rows_input_rect.x + 6, self.rows_input_rect.y + 4)
        )
        ...
        border_color = (
            self.colors["accent"] if self.cols_input_focused else self.colors["border"]
        )
        pygame.draw.rect(
            self.screen, border_color, self.cols_input_rect, border_radius=4
        )
        pygame.draw.rect(
            self.screen,
            self.colors["input_bg"],
            self.cols_input_rect.inflate(-2, -2),
            border_radius=4,
        )
        ...
        self.screen.blit(
            cols_surf, (self.cols_input_rect.x + 6, self.cols_input_rect.y + 4)
        )
        ...
        pygame.draw.rect(
            self.screen, button_color, self.apply_button_rect, border_radius=4
        )
        ...
        pygame.draw.rect(
            self.screen, button_color, self.toggle_grid_button_rect, border_radius=4
        )
        toggle_text = self.font_main.render(
            "Grid: ON" if self.show_grid_overlay else "Grid: OFF",
            True,
            self.colors["text"],
        )
