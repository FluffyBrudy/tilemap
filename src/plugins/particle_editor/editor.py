...
class ParticleEditor:
    ...
    def _switch_library(self, label: str) -> None:
        ...
        if first is not None:
            self.load_entry(first.name, copy.deepcopy(first.config))
        self._rebuild_thumbs()
    ...
    def load_entry(self, name: str, config: dict) -> None:
        ...
        self.loaded_config = copy.deepcopy(normalized)
        ...
        self._build_controls()
        self.color_picker.sync_from_config(self.working)
        self.sim.reset(copy.deepcopy(normalized))
        self.layout()
        for w in warnings:
            self.toasts.warning(w)

    def revert(self) -> None:
        self.working = copy.deepcopy(self.loaded_config)
        ...
        self._build_controls()
        self.color_picker.sync_from_config(self.working)
        self.sim.reset(copy.deepcopy(self.working))
        self.say("Reverted")
    ...
    def save(self) -> None:
        ...
        if existing is not None:
            existing.config = copy.deepcopy(self.working)
        else:
            library.systems.append(ParticleSystemEntry(name=name, config=copy.deepcopy(self.working)))
        try:
            library.save(target)
        except OSError as e:
        ...
        if target != self.library_path:
            ...
            self._refresh_libraries()
        self.loaded_name = name
        self.loaded_config = copy.deepcopy(self.working)
        self.name_field.set_value(name)
