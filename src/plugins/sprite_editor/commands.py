...
class FlipCommand(Command):
    ...
    Replace-in-place block mirror (arrangement reverses along each
    flipped axis, pixels flip too) — never a stacked copy, never canvas
    growth. Single-cell selections behave like a plain tile flip.
    Sparse selections mirror within their bounding box.
    """
...
class CropCommand(Command):
    ...
    Tile-snapped via ``trim_to_content``; records whether anything
    changed so callers can discard the no-op push.
    """
    ...
    def _do(self, doc: Document, selection: Selection) -> None:
        oc, orow = doc.origin_col, doc.origin_row
        self.changed = doc.trim_to_content()
        if not self.changed:
            return
        dx, dy = doc.origin_col - oc, doc.origin_row - orow
        if dx or dy:
            max_c, max_r = doc.cols - 1, doc.rows - 1
            selection.replace([
                (c - dx, r - dy)
                for c, r in selection.sorted_cells()
                if 0 <= c - dx <= max_c and 0 <= r - dy <= max_r
            ])
...
class PixelScaleCommand(Command):
    ...
    def __init__(self, src_rect: Rect, dest_rect: Rect):
        self.src_rect = Rect(src_rect)
        self.dest_rect = Rect(
            int(dest_rect.x), int(dest_rect.y),
            max(1, int(dest_rect.w)), max(1, int(dest_rect.h)),
        )
        self.dest: Rect | None = None
