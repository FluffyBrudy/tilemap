...
class Viewport:
    ...
    def _draw_sheet(self, screen: Surface) -> None:
        ...
        if sw <= MAX_CACHE_SIZE and sh <= MAX_CACHE_SIZE:
        ...
        cached_size = self._last_sheet.get_size() if self._last_sheet is not None else None
        if (true_w, true_h) != cached_size:
            try:
    ...
    def _draw_canvas_bounds(self, screen: Surface) -> None:
        ...
        if right_w > 0:
            draw_alpha_fill(
                screen,
                Rect(dest.right, content.y, right_w, content.h),
                (0, 0, 0),
                dim,
            )
        ...
        o = dest.topleft
        pygame.draw.lines(screen, COLORS.warning, False, [(o[0], o[1] + 10), o, (o[0] + 10, o[1])], 3)
        br = dest.bottomright
        tick = 7
        pygame.draw.line(screen, COLORS.accent_active, (br[0] - tick, br[1]), (br[0] + tick, br[1]), 2)
        pygame.draw.line(screen, COLORS.accent_active, (br[0], br[1] - tick), (br[0], br[1] + tick), 2)
