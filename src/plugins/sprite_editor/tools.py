...
class SelectTool(Tool):
    ...
    def __init__(self, ctx: ToolContext):
        ...
        # None = on canvas, "blocked" = past the top/left wall,
        # "expand" = past the live bottom/right edge (hint-only)
    ...
    def draw_overlay(self, screen: Surface) -> None:
        if self._mode == "move":
            ...
            # hint-only cursor marker past the canvas edge: warning for the
            # blocked top/left wall, accent for the expandable sides
...
class FreeTool(Tool):
    ...
    def __init__(self, ctx: ToolContext):
        ...
        # Tight marquee: shrink the committed block to the content
        # bounding box (transparent counts as empty). Default OFF.
        self.tight: bool = False
        # floating pixel paste (armed by Ctrl+V with a pixel clipboard):
        # ghost follows the cursor until LMB/Enter stamps it.
        self._floating: Surface | None = None
        self._floating_pos: tuple[int, int] = (0, 0)
        # stretch drag: handle id, pressed block anchor and pending dest
    ...
    # -- editor-facing accessors (copy/cut/paste route through these) --
    ...
    def handle_event(self, event: pygame.event.Event) -> bool:
        # RMB cancels an armed pixel paste; it must win over base-class
        # RMB-pan or cancel attempts just pan the view instead
        if (
            event.type == pygame.MOUSEBUTTONDOWN
            and event.button == 3
            and self._floating is not None
        ):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if (
                event.key in (pygame.K_h, pygame.K_v)
                and self._drag is None
                and self._floating is None
            ):
    ...
    def _update_resize(self, pos: tuple[int, int]) -> None:
        ...
        x0, y0, w0, h0 = (self._press_block.x, self._press_block.y,
                          self._press_block.w, self._press_block.h)
    ...
    def _on_left_up(self) -> None:
        ...
        if drag == "marquee":
            ...
            if (dx != 0 or dy != 0) and self._block is not None:
                self.ctx.commands.push(
                    PixelMoveCommand(self._block, dx, dy), self.ctx.doc, self.ctx.selection
                )
    ...
    def _mirror(self, axis: str) -> bool:
        ...
        if cmd.dest is not None:
            self._block = cmd.dest
            where = {("h", 1): "right", ("h", -1): "left",
                     ("v", 1): "down", ("v", -1): "up"}[(axis, side)]
    ...
    def draw_overlay(self, screen: Surface) -> None:
        ...
        if (
            self._drag == "resize"
            and self._resize_ghost is not None
            and self._press_block is not None
        ):
            b = self._press_block
            src_rect = screen_rect_for(self.ctx.camera, b.x, b.y, b.w, b.h)
            draw_alpha_fill(screen, src_rect, CANVAS_BG, 200)
