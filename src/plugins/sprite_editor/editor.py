...
class SpriteEditor:
            ...
            Menu(
                ...
                [
                    ...
                    MenuAction(
                        ...
                        self._on_export_all,
                        "Ctrl+E",
                        is_enabled=lambda: bool(self.doc.regions) and self.doc.has_canvas,
                    ),
                    MenuSeparator(),
                    MenuAction(
                        "Stack Horizontally",
    ...
    def _build_toolbar(self) -> None:
        ...
        def add_btn(
            text: str, *, tooltip: str = "", on_click=None, tag: str = "", group: str, right: bool = False
        ) -> Button:
        ...
        add_btn("Tile Size", tooltip="Change tile size", on_click=self._on_grid, tag="grid", group=self._GRP_TOOLS)
        ...
        layout.add_widget(
            ...
            BTN_H,
            group=self._GRP_TOOLS,
            priority=self._PRIO[self._GRP_TOOLS],
            label="Text label (T)",
            on_activate=self._on_text,
        )
        tight_btn = add_btn(
            "Tight",
        ...
        layout.add_segmented(
            ...
            232,
            group=self._GRP_MODES,
            priority=self._PRIO[self._GRP_MODES],
            overflow_entries=[
        ...
        layout.add_widget(
            export,
            BTN_W,
            group=self._GRP_EXPORT,
            priority=1000,
            right=True,
            collapsible=False,
            label="Export all regions to PNG (Ctrl+E)",
            on_activate=self._on_export_all,
        )
    ...
    def handle_event(self, event: pygame.event.Event) -> bool:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 3:
            ...
            stationary = press is not None and abs(up_pos[0] - press[0]) <= 4 and abs(up_pos[1] - press[1]) <= 4
    ...
    def _paste_paths_from_clipboard(self) -> bool:
        ...
        if self.doc.has_canvas:
            self.commands.push(
                AppendSheetCommand(combined, names=names, horizontal=self._stack_horizontal),
                self.doc,
                self.selection,
            )
            n = len(surfaces)
    ...
    def _flush_pending_drops(self) -> None:
        ...
        if not valid:
            ...
            return
        if skipped:
            self._status_bar.info(f"Skipped {skipped} unsupported file{'s' if skipped != 1 else ''}")
    ...
    def draw(self, screen: Surface) -> None:
        ...
        for btn in self._buttons:
            if getattr(btn, "visible", True):
                btn.draw(screen)
        if self._mode_indicator is not None and getattr(self._mode_indicator, "visible", True):
    ...
    SHORTCUTS = [
        ...
        ("Select All", "Ctrl+A"),
        ("Deselect", "Esc"),
        ("Free pixels", "Free mode: drag select, drag move, Enter region"),
