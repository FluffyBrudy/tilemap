...
class PatternCell:
    ...
    def matches(self, actual_tile_id: Optional[int], actual_tileset_index: Optional[int] = None) -> bool:
        ...
        if self.match_mode == MatchMode.WILDCARD:
            ...
            return (self.tile_id == actual_tile_id and 
                    self.tileset_index == actual_tileset_index)
...
class PatternGrid:
    ...
    def get_cell(self, x: int, y: int) -> PatternCell:
        ...
        return self.cells.get(
            (x, y),
            PatternCell(None, None, MatchMode.WILDCARD)
        )
...
class AutomapEngine:
    ...
    def scan_layer_for_pattern(self, layer: "Layer", pattern: PatternGrid) -> List[Tuple[int, int]]:
        ...
        logging.debug(f"Scanning layer bounds: x=[{min_x}, {max_x}], y=[{min_y}, {max_y}]")
    ...
    def apply_pattern_at_position(self, layer: "Layer", x: int, y: int, pattern: PatternGrid) -> None:
        ...
        for py in range(pattern.height):
            for px in range(pattern.width):
                ...
                if pattern_cell.match_mode == MatchMode.EXACT:
                    ...
                    if pattern_cell.tileset_index is not None and pattern_cell.tileset_index < 0:
                        logging.warning(
                            f"Invalid tileset index {pattern_cell.tileset_index} at pattern position ({px}, {py}), skipping cell"
                        )
                        continue
                    
                    if pattern_cell.tile_id is not None and pattern_cell.tileset_index is not None:
                        # Create tile data
                        tile_data = {
                            "pos": (layer_x, layer_y),
