...
class PropertyEditor:
    def __init__(self, editor: "Editor", title: str, properties: Dict[str, Any], on_save: Callable[[Dict[str, Any]], None], on_close: Callable[[], None], shrink_value_font: bool = False):
        ...
        self.width = 400
        self.height = 500
        self.rect = Rect((editor.width - self.width) // 2, (editor.height - self.height) // 2, self.width, self.height)
    ...
    def draw(self, screen: Surface):
        ...
        # Content Area
        content_rect = Rect(self.rect.x, self.rect.y + 40, self.width, self.height - 90)
        pygame.draw.line(screen, (60, 64, 69), (self.rect.x, self.rect.y + 40), (self.rect.right, self.rect.y + 40))
        ...
        if self.is_entering_new_key:
            ...
            key_surf = self.font_label.render("New Key: " + self.new_key_input + "|", True, (255, 255, 255))
            screen.blit(key_surf, (row_rect.x + 10, row_rect.y + 10))

        screen.set_clip(clip)
        
        # Buttons
        for btn, text, col in [(self.btn_save, "Save", (40, 150, 80)), 
