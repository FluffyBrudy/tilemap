...
class Region:
    ...
    id: str
    rect: Rect  # In image coordinates (unzoomed)
    name: str = ""
...
class RegionSelector:
    ...
    def __init__(
        ...
        self.scroll_y = 0
        self._panning = False
        self._pan_mode = False  # Toggle: when True, left-click drags pan instead of creating/selecting
    ...
    def _clamp_scroll(self) -> None:
        ...
        # If image smaller than viewport, center it (no scroll needed)
        if img_w <= vp_w:
            self.scroll_x = 0
        else:
    ...
    def _get_handle_at(self, screen_pos: Tuple[int, int], region: Region) -> ResizeHandle:
    ...
    def handle_event(self, event: pygame.event.Event) -> bool:
        ...
        # Pan with middle mouse (always) or left-click when pan_mode is on
        if event.type == pygame.MOUSEBUTTONDOWN:
            # Pan toggle button click
            if event.button == 1 and hasattr(self, '_pan_btn_rect') and self._pan_btn_rect.collidepoint(mouse):
                self._pan_mode = not self._pan_mode
                if not self._pan_mode and self._panning:
                    self._panning = False
                return True
        ...
        if event.type == pygame.MOUSEBUTTONUP:
            if (event.button == 2 or (event.button == 1 and self._pan_mode)) and self._panning:
                self._panning = False
                return True
        ...
        # Keyboard zoom (+/-) — works regardless of mouse position
        if event.type == pygame.KEYDOWN:
            old_zoom = self.zoom
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
            ...
            # Check for handle hit on selected region
            ...
            # Start creating new region (on image only, but not when pan_mode is on)
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            if self._dragging and self.selected_id:
                ...
                if region:
                    ...
                    if self.image:
                        img_w = self.image.get_width()
                        img_h = self.image.get_height()
                        region.rect.x = max(0, min(region.rect.x, img_w - region.rect.width))
                        region.rect.y = max(0, min(region.rect.y, img_h - region.rect.height))
            ...
            if self._resizing and self.selected_id:
                ...
                if region:
                    ...
                    if self._hover_handle in (ResizeHandle.LEFT, ResizeHandle.TOP_LEFT, ResizeHandle.BOTTOM_LEFT):
                        new_rect.x += dx
                        new_rect.width -= dx
                    if self._hover_handle in (ResizeHandle.RIGHT, ResizeHandle.TOP_RIGHT, ResizeHandle.BOTTOM_RIGHT):
                        new_rect.width += dx
                    if self._hover_handle in (ResizeHandle.TOP, ResizeHandle.TOP_LEFT, ResizeHandle.TOP_RIGHT):
                        new_rect.y += dy
                        new_rect.height -= dy
                    if self._hover_handle in (ResizeHandle.BOTTOM, ResizeHandle.BOTTOM_LEFT, ResizeHandle.BOTTOM_RIGHT):
                        new_rect.height += dy
                    
                    # Enforce minimum size
                    if new_rect.width >= self.MIN_REGION_SIZE and new_rect.height >= self.MIN_REGION_SIZE:
                        # Clamp to image bounds
                        if self.image:
                            img_w = self.image.get_width()
                            img_h = self.image.get_height()
                            new_rect.x = max(0, min(new_rect.x, img_w - self.MIN_REGION_SIZE))
                            new_rect.y = max(0, min(new_rect.y, img_h - self.MIN_REGION_SIZE))
    ...
    def draw(self, screen: Surface) -> None:
        ...
        # Pan mode toggle button (top-left)
        ...
        pan_label = self._font_sm.render("Pan" if not self._pan_mode else "PAN", True,
                                          COLORS.text if self._pan_mode else COLORS.text_dim)
        screen.blit(pan_label, (pan_btn_rect.centerx - pan_label.get_width() // 2,
                                pan_btn_rect.centery - pan_label.get_height() // 2))
        self._pan_btn_rect = pan_btn_rect  # Store for click detection
        
        # Zoom indicator (next to pan button)
        zoom_text = self._font_sm.render(f"Zoom: {self.zoom:.1f}x", True, COLORS.text_dim)
    ...
    def _draw_region(self, screen: Surface, region: Region) -> None:
        ...
        pygame.draw.rect(screen, border_color, r, self.SELECTION_BORDER_WIDTH if is_selected else 1)
