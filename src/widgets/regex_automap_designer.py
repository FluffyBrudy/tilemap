...
class RegexAutomapDesigner:
    ...
    def _create_match_mode_icons(self) -> dict:
        ...
        pygame.draw.circle(wildcard_surf, (80, 80, 100), (icon_size // 2, icon_size // 2), icon_size // 2 - 1)
        pygame.draw.circle(wildcard_surf, (120, 120, 140), (icon_size // 2, icon_size // 2), icon_size // 2 - 1, 1)
        # Draw asterisk
        ...
        pygame.draw.rect(filled_surf, (229, 192, 123), Rect(2, 2, icon_size - 4, icon_size - 4))
        pygame.draw.rect(filled_surf, (180, 150, 90), Rect(2, 2, icon_size - 4, icon_size - 4), 2)
        # Draw F
        f_text = pygame.font.SysFont("Arial", 10, bold=True).render("F", True, (80, 60, 30))
        ...
        for x in range(2, icon_size - 2, dash_length * 2):
            pygame.draw.line(empty_surf, dash_color, (x, 2), (min(x + dash_length, icon_size - 2), 2), 2)
        # Bottom
        for x in range(2, icon_size - 2, dash_length * 2):
            pygame.draw.line(empty_surf, dash_color, (x, icon_size - 3), (min(x + dash_length, icon_size - 2), icon_size - 3), 2)
        # Left
        for y in range(2, icon_size - 2, dash_length * 2):
            pygame.draw.line(empty_surf, dash_color, (2, y), (2, min(y + dash_length, icon_size - 2)), 2)
        # Right
        for y in range(2, icon_size - 2, dash_length * 2):
            pygame.draw.line(empty_surf, dash_color, (icon_size - 3, y), (icon_size - 3, min(y + dash_length, icon_size - 2)), 2)
        # Draw E
        e_text = pygame.font.SysFont("Arial", 10, bold=True).render("E", True, (70, 130, 180))
        ...
        pygame.draw.circle(exact_surf, (152, 195, 121), (icon_size // 2, icon_size // 2), icon_size // 2 - 1)
        pygame.draw.circle(exact_surf, (120, 160, 90), (icon_size // 2, icon_size // 2), icon_size // 2 - 1, 1)
        # Draw checkmark
    ...
    def _update_layout(self):
        ...
        self.apply_btn_rect = Rect(start_x + (btn_w + btn_spacing) * 2, btn_y, btn_w, 30)
        ...
        if tile_selector and tile_selector.selected_tile:
            ...
            if ts:
                ...
                try:
                    ...
                    self.current_preview_surf = None
    ...
    def _cycle_match_mode(self, grid: PatternGrid, x: int, y: int):
        ...
        mode_cycle = [MatchMode.EXACT, MatchMode.WILDCARD, MatchMode.ANY_FILLED, MatchMode.ANY_EMPTY]
        ...
        tileset_index = cell.tileset_index if cell.tileset_index is not None else self.current_tileset_index
    ...
    def _load_rule_to_editor(self, rule: PatternRule):
        ...
        """
        self.input_pattern_grid = PatternGrid(rule.input_pattern.width, rule.input_pattern.height)
        self.output_pattern_grid = PatternGrid(rule.output_pattern.width, rule.output_pattern.height)
    ...
    def _apply_automap(self):
        ...
        if transformation_count >= engine.max_transformations:
            print(f"WARNING: Transformation limit ({engine.max_transformations}) reached!")
            print("This may indicate circular dependencies in your pattern rules.")
            print("Consider reviewing your rules to ensure output patterns don't match input patterns.")
        else:
    ...
    def serialize_rules(self) -> List[dict]:
        ...
        for i, rule in enumerate(self.pattern_rules):
            try:
                ...
                logging.error(f"Error serializing rule '{rule.name}' (index {i}): {e}", exc_info=True)
    ...
    def deserialize_rules(self, rules_data: List[dict]) -> None:
        ...
        for i, rule_dict in enumerate(rules_data):
            try:
                ...
                logging.warning(f"Error deserializing rule at index {i}: {e}", exc_info=True)
    ...
    def _draw_rule_list(self, screen: Surface):
        ...
        screen.blit(btn_text, (self.new_rule_btn_rect.x + 10, self.new_rule_btn_rect.y + 5))
    ...
    def _draw_pattern_grids(self, screen: Surface):
        ...
        screen.blit(output_label, (output_grid_rect.centerx - 55, output_grid_rect.y - 25))
        ...
        if tile_selector and cell.tileset_index is not None:
            if 0 <= cell.tileset_index < len(tile_selector.tilesets):
                ...
                try:
                    ...
                    # Scale to fit cell
                    scaled = pygame.transform.scale(tile_surf, (self.cell_size - 4, self.cell_size - 4))
                    screen.blit(scaled, (cell_rect.x + 2, cell_rect.y + 2))
                except:
                    pass
    
    def _draw_match_mode_indicator(self, screen: Surface, cell: PatternCell, cell_rect: Rect):
    ...
    def _draw_buttons(self, screen: Surface):
        ...
        if self.selected_rule_idx >= 0:
            pygame.draw.rect(screen, (180, 70, 70), self.delete_btn_rect, border_radius=4)
            del_text = self.font.render("Delete", True, Color("white"))
            screen.blit(del_text, (self.delete_btn_rect.x + 20, self.delete_btn_rect.y + 8))
