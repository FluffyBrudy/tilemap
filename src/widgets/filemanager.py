...
class ImagePreview:
    ...
    def draw(self, surface: pygame.Surface, rect: pygame.Rect):
        ...
        # Calculate available space for image (leave room for dimensions text at bottom)
        text_height = 30
        image_area_height = rect.height - close_button_size - close_button_margin * 2 - text_height
...
class FileManager:
    ...
    def refresh_items(self):
        ...
        try:
            ...
            print(f"DEBUG: Current path: {self.current_path}")
            print(f"DEBUG: Allowed exts: {self.allowed_exts}")
            print(f"DEBUG: Found {len(folders)} folders, {len(files)} files")
    ...
    def handle_event(self, event: pygame.event.Event) -> bool:
        ...
        if event.type == pygame.MOUSEMOTION:
            ...
            elif not self.resize_handler.is_dragging:
